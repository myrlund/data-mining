...
public class BaseApriori<V> extends AbstractApriori<V> {
	...
	public void apriori(Double minSupport) {
		...
		int totalCandidates = candidatesLevel1.size();
		int totalFrequentCandidates = frequentCandidatesLevel.size();
		...
		for (int i = 2;; i++) {
			...
			// count candidates and prunes
			totalCandidates += generateCandidateSizeK.size();
			totalFrequentCandidates += frequentCandidatesLevel.size();
		...
		System.out.println("Pruned candidates: "+(totalCandidates - totalFrequentCandidates));
