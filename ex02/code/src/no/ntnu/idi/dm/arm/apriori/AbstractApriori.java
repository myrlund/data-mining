...
public abstract class AbstractApriori<V> {
	...
	public void generateRulesBase(ItemSet<V> frequentItemSet,
		...
		for (ItemSet<V> transaction : transactions) {
			if (transaction.subsumes(frequentItemSet.union(consequent))) {
				x++;
			}
			if (transaction.subsumes(frequentItemSet)) {
				y++;
			}
		}
		rule.setSupport(x / transactions.size());
		rule.setConfidence(x / y);
